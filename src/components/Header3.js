...
function Header({ handleMenuToggle, isMenuOpen }) {
  ...
  return (
    <header className="shadow-lg sticky top-0 flex justify-between items-center z-20 bg-green-700 h-24 ">
      ...
      {isMenuOpen && (
        <div className="sm:hidden bg-green-600 absolute top-6 right-16 bg-opacity-10 z-20">
          <nav className="px-2 pt-2 pb-2 space-y-1 z-20">
            <Link to="/" className="block text-white hover:text-white" onClick={handleMenuToggle}>
              Products
            </Link>

            <Link to="/about" className="block text-white hover:text-white" onClick={handleMenuToggle}>
              About
            </Link>
            <Link to="/contact" className="block text-white hover:text-white" onClick={handleMenuToggle}>
              Contact
            </Link>
